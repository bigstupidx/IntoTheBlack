...
timeCounter+=Time.deltaTime;
	...
	if(timeCounter>trigger&&effectCounter<=thisManyTimes)
		...
		{
		xRotCur=transform.rotation.x+(Random.value*xRotMax*1)-(xRotMax);  // decide rotation
		yRotCur=transform.rotation.y+(Random.value*yRotMax*1)-(yRotMax);  
		zRotCur=transform.rotation.z+(Random.value*zRotMax*1)-(zRotMax);  
